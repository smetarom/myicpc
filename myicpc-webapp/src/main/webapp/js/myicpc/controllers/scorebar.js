...
var scorebar, scorebarDisplayText, scorebarHideText, updateScorebar;
...
scorebar.controller('scorebarCtrl', function($scope) {
  ...
  $scope.updateTeamInfo = function(team) {
    ...
    if (($("#info-" + team.teamId + "-failed").length)) {
      return $("#info-" + team.teamId + "-failed").html(team.failedNum + " " + formatProblemArray(team.failed));
    }
  };
  $scope.drawTeamBar = function(team) {
    ...
    }).attr("class", "failed").on("mouseover", function(team) {
      return alert(team["teamId"]);
  ...
  return $scope.findById = function(teamId) {
...
updateScorebar = function(data, ngController) {
  var effectedTeam, index, key, team, teamId;
