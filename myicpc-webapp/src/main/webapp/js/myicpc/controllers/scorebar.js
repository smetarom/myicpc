...
var scorebar, sortTeamRanks, updateScorebar;
...
scorebar.controller('scorebarCtrl', function($scope) {
  ...
  $scope.getBarWidth = function(team) {
...
sortTeamRanks = function(teams, start, end) {
  var effectedTeams, i, postSorted, preSorted, sorted, _i, _ref;
  if (start == null) {
    start = 0;
  }
  if (end == null) {
  ...
  sorted = teams.slice(start, end).sort(function(t1, t2) {
  ...
  effectedTeams = [];
  for (i = _i = start, _ref = end - 1; _i <= _ref; i = _i += 1) {
    if (!(teams[i].rank != null) || teams[i].rank !== start + i + 1) {
