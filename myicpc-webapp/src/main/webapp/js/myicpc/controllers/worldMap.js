...
scorebar.controller('worldMapCtrl', function($scope) {
  ...
  $scope.updateTeamOnMap = function(team, solved) {
    ...
    if (solved) {
      return d3.select("#map-circle-" + team["teamId"]).transition().duration(500).style("fill", textRankColor(team.rank)).attr("r", 2 * $scope.config.circleSize).transition().duration(2500).transition().duration(500).attr("r", $scope.config.circleSize).each("end", function() {});
    } else {
