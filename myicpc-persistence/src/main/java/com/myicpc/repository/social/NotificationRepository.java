...
public interface NotificationRepository extends PagingAndSortingRepository<Notification, Long> {
    ...
    @Query("SELECT n FROM Notification n WHERE n.id < ?1 AND n.contest = ?3 AND n.deleted = false AND n.offensive = false AND n.notificationType IN ?2 ORDER BY n.id DESC")
    Page<Notification> findByNotificationTypesOrderByIdDesc(Long lastNotificationId, List<NotificationType> notificationTypes, Contest contest, Pageable pageable);
